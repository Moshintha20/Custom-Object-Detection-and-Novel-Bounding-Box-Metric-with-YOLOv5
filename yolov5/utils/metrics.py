...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, FIoU=False, HIoU=False, eps=1e-7):
    """
    ...
    # IoU
    iou = inter / union
    if CIoU or DIoU or GIoU or FIoU or HIoU:
        ...
        if CIoU or DIoU or FIoU or HIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            c2 = cw**2 + ch**2 + eps  # convex diagonal squared
            rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4  # center dist ** 2
            
            if CIoU or HIoU:  # Hybrid IoU includes CIoU logic
                w1, h1 = b1_x2 - b1_x1, b1_y2 - b1_y1
                w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1
                v = (4 / math.pi**2) * (torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2)
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                ciou_loss = iou - (rho2 / c2 + v * alpha)  # CIoU
                
                if HIoU:
                    # Compute FIoU component
                    l2 = (b1_x1 - b2_x1)**2 + (b1_y1 - b2_y1)**2 + (b1_x2 - b2_x2)**2 + (b1_y2 - b2_y2)**2
                    fiou_loss = iou - (l2 / c2)  # FIoU

                    # Sigmoid-based dynamic weighting for smooth transition
                    w_fiou = torch.sigmoid(8 * (1 - iou))  # Higher weight for FIoU when IoU is low
                    w_ciou = 1 - torch.sigmoid(10 * (1 - iou))  # Higher weight for CIoU when IoU is high

                    return w_fiou * fiou_loss + w_ciou * ciou_loss  # HIoU final computation

                return ciou_loss  # CIoU

            elif FIoU:
                l2 = (b1_x1 - b2_x1)**2 + (b1_y1 - b2_y1)**2 + (b1_x2 - b2_x2)**2 + (b1_y2 - b2_y2)**2
                return iou - (l2 / c2)  # FIoU
